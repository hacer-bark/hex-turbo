...
fn encode_matches_reference() {
    for len in lengths() {
        ...
        let n = LOWER_CASE.encode_slice(&input, &mut buf).unwrap();
        assert_eq!(n, len * 2);
        assert_eq!(buf, lower.as_bytes(), "encode_slice mismatch at len {len}");
    }
}
...
fn decode_roundtrips_and_accepts_mixed_case() {
    for len in lengths() {
        ...
        let n = LOWER_CASE.decode_slice(&lower, &mut buf).unwrap();
        assert_eq!(n, len);
        assert_eq!(buf, input, "decode_slice mismatch at len {len}");
    }
}
...
fn rejects_odd_lengths_and_short_buffers() {
    for len in [1usize, 3, 31, 33, 63, 65, 129] {
        ...
        let mut buf = vec![0u8; len / 2];
        assert_eq!(
            LOWER_CASE.decode_slice(&odd, &mut buf),
            Err(Error::InvalidLength),
            "decode_slice len {len}"
        );
    }
    ...
    assert_eq!(
        LOWER_CASE.encode_slice(data, &mut small),
    ...
    assert_eq!(
        LOWER_CASE.decode_slice(&encoded, &mut small),
        Err(Error::BufferTooSmall)
    );
}
...
fn oversized_output_buffer_is_untouched_past_the_written_prefix() {
    ...
    let mut buf = vec![0xAAu8; 400];
    let n = LOWER_CASE.decode_slice(&encoded, &mut buf).unwrap();
    assert_eq!(n, 100);
    assert!(
...
fn large_inputs_match_reference_at_every_destination_alignment() {
    for &len in &[32 * 1024 + 1, 2 * 1024 * 1024 + 1, 2 * 1024 * 1024 + 129] {
        ...
        for off in [0usize, 1, 2, 8, 15, 16, 17, 30, 31] {
            ...
            let n = LOWER_CASE.encode_slice(&input, &mut enc[off..]).unwrap();
            assert_eq!(n, len * 2);
            assert_eq!(
                &enc[off..],
                expected.as_bytes(),
            ...
            let n = LOWER_CASE
                .decode_slice(expected.as_bytes(), &mut dec[off..])
                .unwrap();
            assert_eq!(n, len);
            assert_eq!(
                &dec[off..],
